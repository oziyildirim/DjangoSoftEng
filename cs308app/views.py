...
class categoryFilter(APIView):
    ...
    def post(self, request, id):
        ...
        if request.data["brand"] == "empty":
            if request.data["price_upper"] == "empty":
                if request.data["rating"] == "empty":
                    for ceydacik in ceyda:
                        try:
                    ...
                    for ceydacik in ceyda:
                        try:
                            data = Product.objects.get(product_id=str(ceydacik.product),
                                                       rating__gte=request.data["rating"])
                            serializer = ProductSerializer(data)
                            arr.append(serializer.data)
                        except:
                            continue
                ...
                if request.data["rating"] == "empty":
                    for ceydacik in ceyda:
                        try:
                            data = Product.objects.get(product_id=str(ceydacik.product),
                                                       price__gte=request.data["price_lower"],
                                                       price__lte=request.data["price_upper"])
                            serializer = ProductSerializer(data)
                            arr.append(serializer.data)
                        except:
                            continue
                    ...
                    for ceydacik in ceyda:
                        try:
                            data = Product.objects.get(product_id=str(ceydacik.product),
                                                       rating__gte=request.data["rating"],
                                                       price__gte=request.data["price_lower"], price__lte=request.data["price_upper"])
                            serializer = ProductSerializer(data)
                            arr.append(serializer.data)
                        except:
                            continue
